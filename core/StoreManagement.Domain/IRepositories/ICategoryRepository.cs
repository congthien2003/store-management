...
namespace StoreManagement.Domain.IRepositories
{
    public interface ICategoryRepository<TCategory> where TCategory : Category
    {
        ...
        Task<List<TCategory>> GetAllByIdStoreAsync(int id, int currentPage = 1, int pageSize = 5, string searchTerm = "", string sortCol = "", bool ascSort = true, bool incluDeleted = false);
